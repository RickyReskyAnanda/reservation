      ...
      <div class="box-body table-responsive">
        ...
        <table id="tabel1" class="table table-bordered table-striped">
          ...
          <tbody>
            ...
            <td>{{$venues->venue_type}}</td>
						<td>{{$venues->address}}</td>
						<td>
              @if(isset($venues->getKecamatan->name) && isset($venues->getKota->name) && isset($venues->getProvinsi->name))
              Lokasi : {{ucwords(strtolower($venues->getKecamatan->name.', '.$venues->getKota->name.', '.$venues->getProvinsi->name))}}<br/>
              @endif
              Alamat : {{$venues->address}}
            </td>
						<td>
              @if($venues->sts == '1')
              <span class="label label-success"><b>Ditampilkan</b></span>
              @else
              <span class="label label-danger"><b>Disembuyikan</b></span>
              @endif
            </td>
						<td>
