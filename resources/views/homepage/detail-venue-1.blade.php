...
<html lang="en">
  ...
  <body id="page-top">
                      ...
                      <div class="col">
                        @foreach($detail->getOperationalHours as $oh)
                          @if($oh->day == date('w'))
                            ...
                            <div>Hari Ini <?php if(strlen($oh->start)<2)echo "0";?>{{$oh->start}}:00 -  <?php if(strlen($oh->end)<2)echo "0";?>{{$oh->end}}:00</div>
                            @endif
                          @endif
                        @endforeach
                        <div class="dropdown show">
                          ...
                          @foreach($detail->getOperationalHours as $oh)
                            ...
                            @if($oh->day == date('w'))
                            <div class="dropdown-item"><b class="text-danger">{{$hari[$oh->day]}} <?php if(strlen($oh->start)<2)echo "0";?>{{$oh->start}}:00 -  <?php if(strlen($oh->end)<2)echo "0";?>{{$oh->end}}:00</b></div>
                            @else
                            <div class="dropdown-item">{{$hari[$oh->day]}} <?php if(strlen($oh->start)<2)echo "0";?>{{$oh->start}}:00 -  <?php if(strlen($oh->end)<2)echo "0";?>{{$oh->end}}:00</div>
                            @endif
                          @endforeach
                          </div>
                        </div>
            ...
            <div class="card">
              <div class="card-body">
                ...
                <form action="{{url('review/venue')}}" class="text-right" id="taTulisTanggapan" method="post">
                  {{csrf_field()}}
                  <input type="hidden" name="id"  value="{{base64_encode(base64_encode(strval($detail->id_venue)))}}">
                ...
                <div class="media mb-4">
                  <img class="d-flex mr-3 rounded-circle" src="{{asset('images/icon/review-user.png')}}" alt="">
    ...
    <script type="text/javascript">
     ...
     <?php if(isset($detail->latitude) && isset($detail->longitude)){?>
      function initMap() {
